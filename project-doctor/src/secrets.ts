...
/**
 * Scan for secrets in codebase (from workspaceRoot when provided)
 */
export async function scanSecrets(
  ...
  await scanDirectory(workspaceRoot, normalizedPatterns, tokenPatterns, threshold, result);
...
async function scanDirectory(
  ...
  if (shouldIgnore(normalizedDir, ignorePatterns)) {
    result.ignored++;
    return;
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        await scanDirectory(normalizedPath, ignorePatterns, tokenPatterns, threshold, result);
...
/**
 * Convert glob pattern to regex (basic). Normalize backslashes to forward slashes for Windows.
 */
