...
/**
 * Generate GitHub Actions workflows based on detected stacks (relative to workspaceRoot)
 */
export async function generateWorkflows(
  ...
  for (const stack of stacks) {
    ...
    await generateStackWorkflow(stack, config, workflowConfig, root);
  }

  await generatePrecursorWorkflow(config, stacks, root);
}
...
async function generateStackWorkflow(
  stack: string,
  config: PrecursorConfig,
  workflowConfig: { os?: string[]; matrix?: Record<string, unknown> } | undefined,
  root: string,
  join: (...p: string[]) => string
): Promise<void> {
  const os = workflowConfig?.os || ["ubuntu-latest"];
  const workflowPath = join(root, ".github/workflows", `${stack}.yml`);
...
async function generatePrecursorWorkflow(
  ...
  await mergeFile(join(root, ".github/workflows/precursor.yml"), workflow, { backup: true });
}
