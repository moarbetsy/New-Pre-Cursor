...
/**
 * Get knowledge base file path (relative). Use with resolve(workspaceRoot, path) when workspaceRoot is set.
 */
 ...
 * @param config - Precursor config
 * @param workspaceRoot - If set, create PRECURSOR.md under this path (e.g. when scaffolding another project)
 */
export function initializeKnowledgeBase(config: PrecursorConfig, workspaceRoot?: string): void {
  const knowledgeCfg = config.knowledge || {};
  if (knowledgeCfg.enabled === false) {
    return;
  }

  const relPath = getKnowledgePath(config);
  const path = workspaceRoot ? resolve(workspaceRoot, relPath) : resolve(relPath);
  if (existsSync(path)) {
    return;
  }
