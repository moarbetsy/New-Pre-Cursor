...
export async function runVerification(
  ...
  for (const stack of stacks) {
    ...
    // Collect errors and warnings (allowed failures still reported as failed but policy may allow continuation)
    const allowFailures = verificationCfg.allowFailures || [];
    for (const result of results) {
      if (!result.success) {
        const message = `Verification failed for ${stack}: ${result.command}`;
        const isAllowed = allowFailures.some(pat => result.command.includes(pat));
        const fullMessage = result.error ? `${message}\n${result.error}` : message;
        if (isAllowed) {
