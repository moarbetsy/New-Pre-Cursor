...
import { detectStacks, getWorkspaceRoot } from "./detector.js";
import { runDoctor } from "./doctor.js";
import { runScaffold, assertNoMdcCorruption } from "./scaffold.js";
...
export interface PrecursorOptions {
  ...
  json?: boolean;
  noColor?: boolean;
  /** Workspace root for all generated paths and commands; determined once from config if not set */
...
export async function setup(options: PrecursorOptions = {}): Promise<PrecursorResult> {
  try {
    ...
    // Determine workspace root once (first-class for all operations)
    const workspaceRoot = options.workspaceRoot ?? (await getWorkspaceRoot(config));
    ...
    const toolResults = await resolveAndInstallTools(config, stacks, opts);

    // Generate/update files (relative to workspaceRoot)
    await runScaffold(config, stacks, opts);

    // Post-setup tripwire: fail if any .mdc was written as JSON
    assertNoMdcCorruption(join(workspaceRoot, ".cursor/rules"));

    // Run post-scaffold hooks (formatting, etc.)
    await runPostScaffoldHooks(config, stacks, opts);

    // Run verification loops (cwd = workspaceRoot)
    const verificationReport = await runVerification(config, stacks, opts);
    ...
    if (config.ci?.enabled !== false) {
      await generateWorkflows(config, stacks, opts);
    }

    // Scan for secrets (from workspaceRoot)
    if (config.secrets?.enabled !== false) {
      const secretResults = await scanSecrets(config, workspaceRoot);
