...
/**
 * Detect target Python version from pyproject.toml requires-python or .python-version.
 * Returns 3.14 if target supports 3.14+, else the parsed minor (e.g. 3.11) or null.
 */
export function getTargetPythonVersion(root: string): number | null {
  const pyversionPath = join(root, ".python-version");
  if (existsSync(pyversionPath)) {
    const raw = readFileSync(pyversionPath, "utf-8").trim().split(/\s/)[0];
    const match = raw.match(/^3\.(\d+)/);
    if (match) {
      const minor = parseInt(match[1], 10);
      return minor >= 14 ? 3.14 : 3.11;
    }
  }
  ...
  if (existsSync(pyprojectPath)) {
    ...
    if (requiresMatch) {
      const spec = requiresMatch[1];
      if (/>=3\.14|>=3\.1[4-9]|>=3\.\d{2,}|~=3\.14|==3\.14/.test(spec)) {
        return 3.14;
      }
      const minorMatch = spec.match(/>=3\.(\d+)/);
      if (minorMatch) {
 ...
 */
export async function getWorkspaceRoot(config: PrecursorConfig): Promise<string> {
 ...
 * Detect Python stack
 */
export function detectPython(root: string): boolean {
  const markers = [
    "pyproject.toml",
    "uv.lock",
    "requirements.txt",
