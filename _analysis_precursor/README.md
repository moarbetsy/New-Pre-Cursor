...
**Agent operating protocol:** before doing any work, read `AGENT_PROTOCOL.md` at repo root and follow the step-by-step workflow.
- Log **every step** to `STEPLOG.md` (append-only).
- Use `STEP_TEMPLATE.md` to structure each step.
- Use `REPORT.md` only for real breakages (non-zero exit codes, blocked workflows).
