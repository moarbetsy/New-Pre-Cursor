...
/**
 * Returns true if content looks like a JSON object (e.g. { "content": ... }).
 * Used to flag .mdc files that were wrongly written as JSON.
 */
export function isMdcCorrupted(content: string): boolean {
 ...
 * Must not be used for .mdc files; use plain text write to prevent JSON corruption.
 */
export async function mergeFile(
  ...
  if (filePath.endsWith(".mdc")) {
    throw new Error(
      "mergeFile must not be used for .mdc files; write Markdown directly to prevent JSON corruption."
    );
  }
